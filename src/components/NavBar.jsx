...
const NavBar = () => {
    ...
    return (
        <StyledNavBar>
            ...
            {isAuth && attendee ? (
                <>
                    <NavBarButton buttonText={attendee.firstName}
                                  onClick={() => navigate(`/attendees/profile/${attendee.idAttendee}`)}/>
                    <NavBarButton buttonText="Sign Out" onClick={logoutUser}/>
                </>
            ) : (
                !(isOnLoginPage || isOnRegisterPage) &&
                <NavBarButton buttonText="Sign In" onClick={() => navigate('/login')}/>
            )}
        </StyledNavBar>
    );
};
